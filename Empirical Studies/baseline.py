...
def add2Dict(relation_dict,verb_phrases,sent_dist):
    value = 1
    if sent_dist != 0:
        value = 1.0/sent_dist
    for VP in verb_phrases:
        if VP in relation_dict.keys():
            relation_dict[VP] += value
        else:
            relation_dict[VP] = value
...
def getRelation(entity1,entity2,distance,abstract_list):
    ...
    for abstract in abstract_list:
        ...
        if entity1_count and entity2_count:
            add2Dict(relation_dict,verb_phrases,0)
        ...
        #check sentence before or after distance has missing entity
            ...
            for pointer in xrange(distance):
                ...
                if entity1_count and checkEntityInSent(entity2,pointer+1,abstract,sent_index):
                    add2Dict(relation_dict,verb_phrases,pointer+1)
                    break
    ...
    for vp,freq in relation_dict.items():
        relation_dict[vp] = 0.7*freq + 0.3*math.log(1.0/word_dict[vp])
...
for (entity1,entity2) in entities:
    for dist in dists:
        ...
        write2File(relations,"./baseline/"+entity1+"_"+entity2+"_"+str(dist)+".txt")
